(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('aiProjektApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-aiProjektApp-error');
                var entityKey = httpResponse.headers('X-aiProjektApp-params');
                if (errorHeader) {
